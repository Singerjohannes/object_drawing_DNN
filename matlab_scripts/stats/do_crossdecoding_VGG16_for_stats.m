function [final_photo_drawing_acc,final_drawing_photo_acc, final_photo_sketch_acc,final_sketch_photo_acc, final_drawing_sketch_acc, final_sketch_drawing_acc] = do_crossdecoding_VGG16_for_stats(photo_drawing_activations, drawing_photo_activations, photo_sketch_activations, sketch_photo_activations,drawing_sketch_activations, sketch_drawing_activations, n_iter, category_vector, passed_kernel)
...
% output : mean decoding accuracies for training and testing a combination of conditions (both directions, averaged afterwards) 
% photo-drawing, drawing-photo, photo-sketch,sketch-photo,
% drawing-sketch,sketch-drawing
...
for layer=1:lfn
    ...
    xclass_kernel_drawing_photo = drawing_photo_activations(:,:,layer);
    xclass_kernel_photo_sketch =  photo_sketch_activations(:,:,layer);
    xclass_kernel_sketch_photo =  sketch_photo_activations(:,:,layer);
    xclass_kernel_drawing_sketch = drawing_sketch_activations(:,:,layer);
    xclass_kernel_sketch_drawing = sketch_drawing_activations(:,:,layer);
    ...
    final_drawing_photo_acc(layer) = mean(acc_all(:,layer,2));

    final_photo_sketch_acc(layer) = mean(acc_all(:,layer,3));
    final_sketch_photo_acc(layer) = mean(acc_all(:,layer,4));
    
    final_drawing_sketch_acc(layer) = mean(acc_all(:,layer,5));    
    final_sketch_drawing_acc(layer) = mean(acc_all(:,layer,6));
