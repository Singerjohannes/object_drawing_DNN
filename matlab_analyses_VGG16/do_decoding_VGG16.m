...
for i = 1:n_iter
    ...
    data_test_all = data_all(testind,:,:);
    label_test = labels(testind);
    
    % train model for every depiction 
    model_photo = svmtrain(label_train, data_train_all(:,:,1) ,'-s 0 -t 0 -c 1 -q'); %#ok<SVMTRAIN>
    model_drawing = svmtrain(label_train, data_train_all(:,:,2) ,'-s 0 -t 0 -c 1 -q'); %#ok<SVMTRAIN>
    model_sketch = svmtrain(label_train, data_train_all(:,:,3) ,'-s 0 -t 0 -c 1 -q'); %#ok<SVMTRAIN>

    % get prediction for all depictions
    predicted_label_photo = svmpredict(label_test,data_test_all(:,:,1),model_photo,'-q');
    predicted_label_drawing = svmpredict(label_test,data_test_all(:,:,2),model_drawing,'-q');
    predicted_label_sketch = svmpredict(label_test,data_test_all(:,:,3),model_sketch,'-q');
